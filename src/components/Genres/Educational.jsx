...
function Educational() {
  ...
  const getEducation= async () => {
    const educationApi = await fetch(`https://api.rawg.io/api/games?key=${process.env.REACT_APP_API_KEY}&genres=educational`);
    const data = await educationApi.json();
    console.log(data);
    localStorage.setItem("educational", JSON.stringify(data.results));
    ...
    return (
      ...
      <div className='md:px-24 px-8  text-[#eeeeee] border-b-2 border-b-[#a9081c] '>
        <h3 className='text-start text-3xl bold py-4'>Educational Games</h3>
        <Splide
            options={{
                arrows:false,
                pagination: false,
